...
def create_trial_mat(fb: int, n_trials_train_mat, train_img_codes, ptiles_all = ptiles_all, dim_scores = dim_scores):
...
def save_train_trials(object_indices = object_indices, anchor_images_examples = anchor_images_examples, stim_imgs_20 = stim_imgs_20, n_trials_train = n_trials_train, n_trials_train_nofb = n_trials_train_nofb, n_anchors_pos = n_anchors_pos, n_blocks_train = n_blocks_train, header = header, path_output = path_output):
    ...
    trial_mat_nofb = create_trial_mat(fb = 0, n_trials_train_mat = n_trials_train_nofb, train_img_codes = train_img_codes_nofb)
    ...
    trial_mat_fb = create_trial_mat(fb = 1, n_trials_train_mat = n_trials_train - n_trials_train_nofb, train_img_codes = train_img_codes_fb)
...
def save_exp_trials(dim_scores = dim_scores, n_blocks_exp = n_blocks_exp, n_trials_per_block = n_trials_per_block, n_trials_fb_per_block = n_trials_fb_per_block, n_trials_nofb_per_block = n_trials_nofb_per_block, trials_fb = trials_fb, trials_nofb = trials_nofb, ptiles_all = ptiles_all, stim_imgs_20 = stim_imgs_20, header = header, path_output = path_output):
...
for dim_id in range(np.size(spose, 1)):
    ...
    stim_imgs_train = save_train_trials()
