...
# load data
spose = np.loadtxt(path + 'data/spose_embedding_49d_sorted.txt')  # load y; path to folder resources
...
for dim_id in range(np.size(spose, 1)):
    ...
    for block in range(n_blocks):
        ...
        trial_mat[0:n_trials_fb_per_block, 1] = [spose[int(stim_imgs_20[i]), dim_id] for i in trials_fb_block]
